...
AUG_NB_AUGMENTATION_PER_IMAGE = int(
    os.getenv('AUG_NB_AUGMENTATION_PER_IMAGE', 1))

DATASET_DIR = "/data/jetson-inference/python/training/detection/ssd/data/bev_test"
DATASET_IMAGES_DIR = "{}/JPEGImages".format(DATASET_DIR)
DATASET_ANNOTS_DIR = "{}/Annotations".format(DATASET_DIR)
DATASET_AUGM_DIR = "/data/jetson-inference/python/training/detection/ssd/data/bev_test_aug"
DATASET_AUGM_IMAGES_DIR = "{}/JPEGImages".format(DATASET_AUGM_DIR)
...
for image_path in images_path:
    ...
    if image_annotation_path is None:
        continue
    ...
    for i in range(0, AUG_NB_AUGMENTATION_PER_IMAGE):
        # This setup of augmentation parameters will pick 1 to 2
        # of the given augmenters and apply them in random order.
        def sometimes(aug): return augmenters.Sometimes(0.5, aug)
        aug_config = augmenters.SomeOf(
            (1, 2),
            [
                augmenters.MotionBlur(k=15),
                augmenters.OneOf([
                    augmenters.Affine(scale=(0.9, 1.2)),
                    ...
                    augmenters.ElasticTransformation(alpha=(0.5, 3.5), sigma=0.25),
                    ]),
            ],
            random_order=True
        )
